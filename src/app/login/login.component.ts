...
export class LoginComponent
  ...
  onSubmit(event: Event) {
    try {
      ...
      self.loader = true;
      self.subscriptions['login'] = self.commonService
        .get('user', '/auth/authType/' + username, {
          noApp: true,
          skipAuth: true,
        })
        .subscribe(
          (res) => {
            ...
            if (res.bot) {
              ...
              if (res.rbacUserToSingleSession && res.sessionActive) {
          ...
          try {
            ...
            if (data.status === 200 && !self.commonService.noAccess) {
              ...
              const expireDate = new Date(
                self.sessionService.getUser(true).expiresIn
              );
  ...
  doLogin() {
    try {
      ...
      self.loader = true;
      self.commonService[self.authType === 'local' ? 'login' : 'ldapLogin'](
        self.form.value
      )
        .then((res) => {
          self.commonService
  ...
  activeSessionWarning() {
    const self = this;
    self.clearSessionModalRef = self.commonService.modal(
      self.clearSessionModal
    );
    self.clearSessionModalRef.result.then(
      (close) => {
        if (close) {
