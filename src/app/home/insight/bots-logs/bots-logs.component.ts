...
@Component({
  selector: 'odp-bots-logs',
  templateUrl: './bots-logs.component.html',
  styleUrls: ['./bots-logs.component.scss']
})
export class BotsLogsComponent implements OnInit, OnDestroy {
    ...
    this.dataSource = {
      getRows: (params: IGetRowsParams) => {
        ...
        if (!this.apiConfig.filter) {
          this.apiConfig.filter = { app: { $in: [this.commonService.app._id, null] }, activityCode: { $regex : 'BOT'} };
        }
