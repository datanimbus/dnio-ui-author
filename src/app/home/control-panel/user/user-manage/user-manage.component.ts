...
export class UserManageComponent implements OnInit, OnDestroy {
  @ViewChild('attributeModal', { static: false })
  attributeModal: TemplateRef<HTMLElement>;
  @ViewChild('resetPasswordModel', { static: false })
  resetPasswordModel: TemplateRef<HTMLElement>;
  ...
  constructor(
    ...
    self.userDetails = self.fb.group({
      ...
      alternateEmail: [
        '',
        [Validators.pattern(/([\w]+@[a-zA-Z0-9-]{2,}(\.[a-z]{2,})+|admin)/)],
      ],
    });
  ...
  onAttributeFormTypeChange(type: any) {
    ...
    this.attributesForm
      .get('value')
      .setValue(type.value === 'Boolean' ? false : null);
  }
  ...
  get initials() {
    ...
    return (
      (name[0].charAt(0) +
        (name.length > 1 ? name[name.length - 1].charAt(0) : '')) as string
    ).toUpperCase();
  }
  ...
  editUserDetails() {
    ...
    if (self.hasPermission('PMUBU')) {
      ...
      self.userDetails.patchValue({
        name: self.user?.basicDetails?.name || '',
      });
      self.userDetails.patchValue({
        phone: self.user?.basicDetails?.phone || '',
      });
      // self.userDetails.patchValue({ username: self.user.username });
      self.userDetails.patchValue({
        alternateEmail: self.user?.basicDetails?.alternateEmail || '',
      });
  ...
  updateDetails() {
    ...
    if (self.userDetails.invalid) {
      ...
      if (self.userDetails.get('alternateEmail').value === '') {
        ...
        self.user.basicDetails.alternateEmail =
          self.userDetails.get('alternateEmail').value;
      }
  ...
  //   openResetPassword() {
  //     const self = this;
  //     self.resetPasswordModelRef = self.commonService.modal(
    ...
    if (data) {
      ...
      if (data.removeAdmin) {
        ...
        self.subscriptions['removeAdminAccess'] = self.commonService
          .put(
            'user',
            `/${this.commonService.app._id}/user/utils/appAdmin/${self.user._id}/revoke`,
            payload
          )
          .subscribe(
            () => {
  ...
  makeAppAdmin() {
    ...
    if (self.hasPermission('PMUG')) {
      ...
      self.subscriptions['makeAdmin'] = self.commonService
        .put(
          'user',
          `/${this.commonService.app._id}/user/utils/appAdmin/${self.user._id}/grant`,
          payload
        )
        .subscribe(
          () => {
  ...
  onGroupToggle(group: any, selected: boolean) {
    ...
    if (selected) {
      this.commonService
        .put(
          'user',
          `/${this.commonService.app._id}/user/utils/addToGroups/${this.user._id}`,
          { groups: [group._id], app: this.commonService.app._id }
        )
        .subscribe(
          () => {
      ...
      this.commonService
        .put(
          'user',
          `/${this.commonService.app._id}/user/utils/removeFromGroups/${this.user._id}`,
          { groups: [group._id], app: this.commonService.app._id }
        )
        .subscribe(
          () => {
  ...
  openAttributeModal(item?: any) {
    const resetValue = this.editMode
      ? this.appService.cloneObject(item)
      : { type: 'String' };
